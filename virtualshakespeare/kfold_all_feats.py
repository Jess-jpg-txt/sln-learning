...
# load data
data_total = pd.read_csv('../data/w_removal_virtualshakespeare', sep=" ", header=None)
data_total.columns = ["n1", "n2", "f1", "f2", "f3", "f4", "f5", "f6", "f7", "l"]

feats = 'all_feature (f1-7)'

features = np.vstack([data_total["f1"], data_total["f2"], data_total["f3"], data_total["f4"], data_total["f5"], data_total["f6"], data_total["f7"]])
